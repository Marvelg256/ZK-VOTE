...
use soroban_sdk::{
    ...
    crypto::bn254::Bn254G1Affine, Env, Symbol, Vec, U256,
};
...
impl ThresholdCrypto {
    ...
    pub fn submit_analytic_contribution(
        ...
        let submitted_key =
            DataKey::AnalyticsSubmitted(dao_id, round_id, contributor.clone());
        ...
        let mut agg: AnalyticsAggregate = env
            .storage()
            .persistent()
            .get(&agg_key)
